...
from .agent_mush_1 import root_agent as agent_mush_1
...
- ドローの可能性は考えないでください
- ハンドがボードに絡んでいる場合のみ、call以上のアクションを検討してください
...
- call の場合はコールに必要な正確な金額を指定する。自分のスタックを超える場合は your_chips を指定する
- raise の場合はレイズ後の合計金額を指定する
- all_in の場合は自身の残りスタック全額(your_chips)を指定する
""",
)
