...
# ↓ ev_tool.py からインポート (名前の衝突を避けるため別名で)
from .ev_tool import expected_value as generic_expected_value

_preflop_value_tool = PreflopHandValueTool()
_preflop_ev_tool = PreflopEVTool()
_postflop_ev_tool = PostflopEVTool() # 修正版がインスタンス化される
...
async def calculate_postflop_win_rate(
    hole_cards: list[str],
    ...
    """
    (postflop_ev_tool.py) モンテカルロ法でポストフロップの勝率(win_rate)とタイ率(tie_rate)を計算します。
    
    Args:
        hole_cards: 自分のホールカード (例: ["Ah", "Kd"])
        community_cards: 場に出ているコミュニティカード (例: ["Jc", "Ts", "2d"])
        num_opponents: 対戦相手の数
        trials: モンテカルロの試行回数

    Returns:
        (win_rate, tie_rate) のタプル
    """
    return _postflop_ev_tool.monte_carlo_win_rate(
        hole_cards=list(hole_cards),
...
async def calculate_postflop_ev(win_rate: float, pot: float, call_amount: float) -> float:
    """
    (postflop_ev_tool.py) ポストフロップの期待値を計算します。
    EV = win_rate * pot - (1-win_rate)*call_amount

    Args:
        win_rate: 勝率 (0.0 - 1.0)
        pot: 現在のポットサイズ
        call_amount: コールに必要な額

    Returns:
        期待値 (EV)
    """
    return _postflop_ev_tool.expected_value(
        win_rate=float(win_rate),
        pot=float(pot),
        call_amount=float(call_amount)
    )

async def calculate_generic_ev(win_rate: float, pot_size: float, call_amount: float) -> float:
    """
    (ev_tool.py) コールした場合の汎用的な期待値を計算します。
    ...
    Returns:
        期待値 (EV)
    """
    return float(generic_expected_value(float(win_rate), float(pot_size), float(call_amount)))
...
__all__ = [
    ...
    "compute_preflop_ev",
    "calculate_pot_odds",
    "calculate_postflop_win_rate", # 追加
    "calculate_postflop_ev",       # 追加
    "calculate_generic_ev",        # 追加
]