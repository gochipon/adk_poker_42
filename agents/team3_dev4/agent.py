...
from .tools.calc_pot_odds import calc_pot_odds,calc_pot_odds_from_game_state,analyze_pot_odds_decision
...
strategy_agent = Agent(
    ...
    tools=[calculate_position, hand_evaluation, calc_pot_odds,calc_pot_odds_from_game_state,analyze_pot_odds_decision],
    ...
    instruction="""
					...
					5. analyze_pot_odds_decision : ポットオッズと推定勝率からコール判断を分析。期待値（EV）と判断理由を返す

					これらの情報・結果から、以下の点を考慮しつつ戦略分析を行なってください:
					- 勝率が高い場面では、積極的にレイズをしてください。
					- 強い手札であってもポットオッズと期待値が低い場合は、「損切り」を徹底してください。
					- 「弱い手札 + ポジション良」では、パッシブ（チェック/コール）を意識してください。
